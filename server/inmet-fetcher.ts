...
export async function fetchInmetObservation(
  ...
  try {
      ...
      try {
        ...
        if (nomRes.ok) {
          ...
          if (Array.isArray(list) && list.length > 0) {
            ...
            ibgeCode =
              tags["IBGE:GEOCODIGO"] || tags["ibge_code"] || tags["ibge"];
          }
        }
      } catch (nomErr) {
    ...
    console.log(
      `[INMET Fetcher] Querying INMET API for IBGE code ${ibgeCode}: ${inmetUrl}`,
    );
    ...
    const res = await fetch(inmetUrl, {
      ...
      headers: {
        "User-Agent":
          "Mozilla/5.0 (Windows NT 10.0; Win64; x64) ClimaAgora/1.0",
      },
    });
    clearTimeout(timer);
      ...
      if (cityObj) {
        ...
        if (manha) {
          const result: InmetObservation = {
            ...
            tempMax:
              typeof manha.temp_max === "number"
                ? manha.temp_max
                : parseFloat(manha.temp_max),
            tempMin:
              typeof manha.temp_min === "number"
                ? manha.temp_min
                : parseFloat(manha.temp_min),
            windDirection: manha.dir_vento || "N/A",
            windSpeed: manha.int_vento || "Normal",
            source:
              "INMET - Instituto Nacional de Meteorologia (Governo Federal)",
          };
          console.log(
            `[INMET Fetcher] Successfully retrieved INMET data for ${manha.entidade} (${manha.uf})`,
          );
    ...
    console.warn(
      `[INMET Fetcher] INMET API call failed or timed out:`,
      err?.message || err,
    );
  }
