...
const PRESET_IBGE_CODES: Record<string, string> = {
  "alagoinhas": "2900702", "salvador": "2927408", "feira de santana": "2910800",
  "sao paulo": "3550308", "rio de janeiro": "3304557", "brasilia": "5300108",
  "belo horizonte": "3106200", "curitiba": "4106902", "porto alegre": "4314902",
  "recife": "2611606", "fortaleza": "2304400", "belem": "1501402", "manaus": "1302603",
  "goiania": "5208707", "cuiaba": "5103403", "campo grande": "5002704", "vitoria": "3205309",
  "florianopolis": "4205407", "maceio": "2704302", "aracaju": "2800308", "natal": "2408102",
  "joao pessoa": "2507507", "teresina": "2211001", "sao luis": "2111300", "macapa": "1600303",
  "porto velho": "1100205", "boa vista": "1400100", "rio branco": "1200401", "palmas": "1721000",
  "camacari": "2905701", "vitoria da conquista": "2933307", "ilheus": "2913606",
  "juazeiro": "2918407", "barreiras": "2903201", "itabuna": "2914802", "lauro de freitas": "2919207",
};
...
export async function fetchInmetObservation(
  ...
  try {
    ...
    const cleanCity = (cityName || "").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim();

    for (const [key, code] of Object.entries(PRESET_IBGE_CODES)) {
      if (cleanCity.includes(key)) { ibgeCode = code; break; }
    }
    ...
    if (!ibgeCode && cityName) {
      try {
        ...
        const nomRes = await fetch(nomUrl, { headers: { "User-Agent": "ClimaAgora/1.0 (admmeuarmazem@gmail.com)" } });
        if (nomRes.ok) {
    ...
    if (!ibgeCode) {
      const fallback: InmetObservation = { available: false, source: "INMET (Instituto Nacional de Meteorologia)", reason: "Município ou estação INMET não identificada para este local" };
    ...
    const res = await fetch(inmetUrl, { signal: controller.signal, headers: { "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) ClimaAgora/1.0" } });
    clearTimeout(timer);
      ...
      if (cityObj) {
        ...
        if (manha) {
          const result: InmetObservation = {
            available: true, stationName: `Estação / Município INMET — ${manha.entidade} (${manha.uf})`,
            entity: manha.entidade, uf: manha.uf, ibgeCode,
            ...
            windDirection: manha.dir_vento || "N/A", windSpeed: manha.int_vento || "Normal",
  ...
  const fallbackResult: InmetObservation = { available: false, source: "INMET (Instituto Nacional de Meteorologia)", reason: "Rede INMET indisponível temporariamente para este quadrante" };
